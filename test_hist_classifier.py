...
class HistogramClassifier(object):
    ...
    def addReference(self, image, label):
        ...
        if label not in self._references:
            ...
            self._references[label] += [hist]
        print(f"class_for_hist: {type(self._references[label])} and label : {label}")
    ...
    def deserialize(self, path):
        ...
        for key in list(self._references.keys()):
            ...
            print(f"key_inside : {key} and {type(value)}")
...
def main():
    ...
    path = r'C:\Users\himan\OneDrive\Desktop\OpenCV-4-for-Secret-Agents-Second-Edition\Chapter002'

    # 'Stalinist, interior' reference images
    classifier.addReferenceFromFile(os.path.join(path, 'images/communal_apartments_01.jpg'),
            'Stalinist, interior')
    classifier.serialize('classifier.mat')
    classifier.deserialize('classifier.mat')
    # classifier.classifyFromFile(os.path.join(path, 'images/dubai_damac_heights.jpg'))
    # classifier.classifyFromFile(os.path.join(path, 'images/communal_apartments_01.jpg'))
