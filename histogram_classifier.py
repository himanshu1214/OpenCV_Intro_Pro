    ...
    def classify(self, query_image, query_image_name=None):
        ...
        for label, hist_list in self._references.items():
            ...
            for hist in hist_list:
                similarity += cv2.compareHist(hist.todense(), query_hist,
        ...
        if self.verbose:
            print("##########                  Classification ended here                ###############")
        return b_label
...
def main():
    ...
    for file in list_files:
        ...
        classifier.add_reference_from_file(os.path.join(path, file),
                                           image_name)
        classifier.serialize('classifier.mat')
        classifier.deserialize('classifier.mat')

        print(classifier.classify_from_file(os.path.join(path, 'dubai_damac_heights.jpg')))
        print(classifier.classify_from_file(os.path.join(path, 'communal_apartments_01.jpg')))
